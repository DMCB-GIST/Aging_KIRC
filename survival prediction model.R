...
protein_coding_ensembl <- read.table("./data/protein_coding_ensembl.txt", sep="\t", header=FALSE)$V1
...
GDCdownload(query, directory = getwd())
...
inc_gene <- read.table("./data/requirements/KIRC_inc.txt", header = FALSE)$V1
dec_gene <- read.table("./data/KIRC_dec.txt", header = FALSE)$V1
...
clinical <- read.table("./data/kirc_tcga_clinical_data.tsv", sep = "\t", header = TRUE)
